/*
    ...
    5. 형식
        ...
        HEVING 그룹조건식                                       -- HAVING절은 단독 사용 불가하며, GROUP BY절과 함께 사용해야함.
         ORDER BY 정렬할칼럼 ASC|DESC, 정렬할칼럼 ASC|DESC, ...
*/

-- 1. 부서 테이블의 모든칼럼을 조회하시오.
SELECT DEPT_NO, DEPT_NAME, LOCATION
  FROM DEPARTMENT_T;
  
SELECT *                -- 모든 칼럼을 의미하는 *, 실제 개발할때는 사용 금지(성능 저하됨!)
  FROM DEPARTMENT_T;
  
SELECT DEPARTMENT_T.DEP_NO, DEPARTMENT_T.DEP_NAME, DEPARTMENT_T.LOCATION
  FROM DEPARTMENT_T;

SELECT D.DEPT_NO, D.DEPT_NAME, D.LOCATION
  FROM DEPARTMENT_T D; -- 테이블의 별명(ALIAS)을 D로 지정
  
-- 2. 부서 테이블의 모든칼럼을 조회하시오. 조회할 칼럼명을 '부서번호', '부서명', '위치'로 지정하시오.
...
/*
    ...
    4. 칼럼 IS NULL
    5. 칼럼 IS NOT NULL
    6. 칼럼 LIKE 값 : 값이 포함된 것을 찾음 (일치하는 값)
*/
