...
/*
    트랜잭션
    1. 한 번에 수행해야 하는 작업을 의미한다. (원자성, ALL or Nothing) - 둘 다 하던지 둘 다 안하던지
...
-- 모든 칼럼을 작성하는 경우에는 칼럼리스트 생략 가능. (DEPT_NO, DEPT_NAME, LOCATION)

-- 사원 테이블에 행 삽입하기
INSERT INTO EMPLOYEE_T VALUES(EMPLOYEE_SEQ.NEXTVAL, '구창민', 1, '과장', 'M', '95/05/01', 5000000);-- DATE는 연,월,일 순으로 /(슬래시)가 Default이며,날짜는 하이픈(-) 또는 슬래시(/)로 구분
...
/*
    ...
    UPDATE 테이블명 SET 업데이트할 내용, 업테이트할 내용, ... WHERE 조건식
*/
...
/* SET 절의 = 과 WHERE 절의 = 은 다르다. 
 SET 절 : '인천' 데이터를 LOCATION 으로 넣기
 WHERE 절 : DEPT_NO 는 3
*/
...
UPDATE EMPLOYEE_T
 ...
 -- 롤백
 ROLLBACK; -- 이전 COMMIT 으로 돌아가서 수정사항 1,2번 취소됨.
...
-- 3. DEPARTMENT_T에서 부서번호(DEPART_NO)가 2인 부서를 삭제하시오. (부서번호가 2인 사원이 있다. 부서가 없어지면 사원 정보가 참조 무결성에 위배된다. 참조 무결성 위배에 대비해서 ON DELETE SET NULL 옵션을 주었으므로 사원 테이블의 부서번호가 NULL로 함께 수정된다.)
DELETE 
  FROM DEPARTMENT_T 
 WHERE DEPT_NO = 2;
