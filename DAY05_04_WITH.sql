...
SELECT EMP_NO, NAME, POSITION, GENDER, HIRE_DATE, SALARY
  FROM (SELECT RANK() OVER (ORDER BY SALARY DESC) AS RN, EMP_NO, NAME, POSITION, GENDER, HIRE_DATE, SALARY -- RANK() : 순위 구하기 (동점자 처리)
          FROM EMPLOYEE_T)
 WHERE RN BETWEEN 2 AND 3;         

-- 2. WITH 절을 이용하여 연봉 2 ~ 3위 사원 정보를 조회하시오.
WITH RANKING_T 
AS (SELECT RANK() OVER (ORDER BY SALARY DESC) AS RN, EMP_NO, NAME, POSITION, GENDER, HIRE_DATE, SALARY 
      FROM EMPLOYEE_T) -- 서브 쿼리를 RANKING_T로 부른다.
SELECT EMP_NO, NAME, POSITION, GENDER, HIRE_DATE, SALARY
  FROM RANKING_T
 WHERE RN BETWEEN 2 AND 3; 
