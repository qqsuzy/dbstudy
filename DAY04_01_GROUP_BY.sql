...
-- 1. 사원 테이블에서 부서번호를 기준으로(부서번호별) 그룹화하여 조회하시오.
SELECT DEPART
  FROM EMPLOYEE_T
 GROUP BY DEPART; -- DEPART절을 명시하였으니 SELECT 절에는 DEPART만 조회 가능하다. 
                  -- GROUP BY절과 SLECT절에 칼럼이 같아야 하는건 아니다. (GROUP BY절 : 전체집합 , SLECT절 : 부분집합)
...
/*
    ...
    3. 일반 조건은 WHERE 절에 작성한다.(WHERE 절에서는 통계 함수 사용하면 안됨)
