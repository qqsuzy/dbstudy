...
/*
    ...
    2. 테이블이 필요 없는 조회(DQL)의 경우에는 FROM절까지 작성해야 하는데 이 때 사용한다.
...
/*
    1. 문자 -> 숫자 변환
        TO_NUMBER(문자) 
*/
SELECT '100', TO_NUMBER('100')  -- 조회하면 문자는 왼쪽 정렬 숫자는 오른쪽 정렬된다. 
  FROM DUAL;
...
-- 이렇게 바뀐 뒤에 실행된다. (오라클에서 자체적으로 바꾸어 실행됨)
SELECT *
  FROM DEPARTMENT_T
...
/*
    4. 날짜 -> 문자 변환
        TO_CHAR(날짜, [형식]) -- 내가 원하는 모양의 날짜 형식을 만들 때 (TO_DATE와 헷갈리지 않도록 하기!)
        
        * 현재 날짜/시간 함수 -- 오라클을 설치한 컴퓨터 날짜/시간
