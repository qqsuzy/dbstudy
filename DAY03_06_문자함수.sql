...
-- 2) CONCAT(A, B) : 연결 함수   (인수 2개만 적용 가능)
SELECT 'A' || 'B' || 'C'
     , CONCAT(CONCAT('A', 'B'), 'C') -- CONCAT('A', 'B', 'C') 는 인수 적용 가능 개수 초과로 오류 발생됨
  FROM DUAL;
...
SELECT SUBSTR(NAME, 1, 1) || '*' || SUBSTR(NAME, 3) AS 이름 -- SUBSTR(NAME, LENGTH(NAME)) 이름이 3글자 이상일 경우에 왼쪽과 같이 적용 필요함
  FROM EMPLOYEE_T;
   
-- 6. 찾아 바꾸기
SELECT REPLACE(DEPT_NAME, '부', '팀') AS 부서  -- '부'를 '팀'으로 바꾸기
  FROM DEPARTMENT_T;
SELECT REPLACE(DEPT_NAME, '부', '') AS 부서    -- '부'를 ''으로 바꾸기 ('부'를 지우기/비문자열로 바꾸기)
  FROM DEPARTMENT_T;
...
SELECT LPAD(NAME, 10, '*') --10자리의(한글은 5자리)의 NAME 반환, 왼쪽에 '*' 채움 (필드폭 한글 한 글자는 알파벳 두 글자)
